...
class DeleteUserService:
    def delete(self, user: User) -> None:
        ...
        Media.objects.filter(user=user).update(status=Media.STATUS_DELETED)
        Comment.objects.filter(user=user).delete()
        Conversation.objects.filter(user=user).update(status=Conversation.STATUS_DELETED)

        # TODO maybe send request to payment provider to cancel subscription
        Subscription.objects.filter(user=user).update(status=PaymentEnum.STATUS_CANCELED)
